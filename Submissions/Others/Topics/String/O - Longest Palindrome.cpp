...
int32_t main() {
  ...
  int l = 0, r = odd.size() - 1, odd_max = 0;
  while (l <= r) {
    int mid = (l + r) >> 1;
    if (ok(odd[mid])) {
      odd_max = odd[mid];
  ...
  l = 0, r = even.size() - 1; int even_max = 0;
  while (l <= r) {
    int mid = (l + r) >> 1;
    if (ok(even[mid])) {
      even_max = even[mid];
  ...
  int max_palindrome = max(even_max, odd_max);
  for (int i = 1; i + max_palindrome - 1 <= n; i++) {
    if (hash_s.is_palindrome(i, i + max_palindrome - 1)) {
      cout << s.substr(i - 1, max_palindrome) << '\n';
      break;
    }
  }
