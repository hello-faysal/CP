...
int lcp(int i, int j, int x, int y) {
  ...
  while (l <= r) {
    int mid = (l + r) >> 1;
    if (hash_s.get_hash(i, i + mid - 1) == hash_s.get_hash(x, x + mid - 1)) {
      ans = mid;
      l = mid + 1;
    }
    else {
...
bool cmp(pair<int, int> a, pair<int, int> b) {
  int x = compare(a.first, a.second, b.first, b.second);
  return x == -1;
}
...
int32_t main() {
  ...
  while (cin >> s) {
    ...
    for (int i = 1; i <= n; i++) {
      for (int j = i + 1; j <= n; j++) {
        if (hash_s.is_palindrome(i, j)) {
          if (se.find(hash_s.get_hash(i, j)) == se.end()) {
            v.push_back({i, j});
            se.insert(hash_s.get_hash(i, j));
          }
        }
      }
    }
